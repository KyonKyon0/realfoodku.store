...
try {
    ...
    $sql = "SELECT p.id, p.nama_produk, p.merk, p.image_url, p.deskripsi_singkat, p.harga_rentang_rupiah,
            ...
            WHERE p.kategori = 'Makanan' AND (
                p.nama_produk LIKE :nugget
                OR p.nama_produk LIKE :sosis
                OR p.nama_produk LIKE :bakso
            )
            ORDER BY {$orderBy}";
...
<head>
    ...
    <title>Kumpulan Produk Makanan</title>
    <style>
...
<div class="wrap">
    ...
    <h1 class="title">Kumpulan Produk Makanan</h1>
    <p class="subtitle">Halaman ini menampilkan grup Nugget, Sosis, dan Bakso dengan fitur sorting.</p>
    ...
    <?php elseif (empty($products)): ?>
        <div class="alert">Belum ada produk makanan untuk ditampilkan.</div>
    <?php else: ?>
        <div class="grid">
            <?php foreach ($products as $product): ?>
                <?php $fallbackImage = 'https://images.unsplash.com/photo-1498837167922-ddd27525d352?w=1200&q=80'; ?>
                <article class="card">
